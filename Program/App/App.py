...
for bus_station in bus_station_list:
    ...
    if api_data_error_check_return_value != None:
        ...
        if arvl_bus_data["response"]["msgBody"]["busArrivalList"] == None:
            ...
            bus_station.arvl_bus_list.append(
                ArvlBus(
                    flag,
                    locationNo,
                    lowPlate,
    ...
    for arvl_bus in bus_station.arvl_bus_list:
        ...
        routeNowStaName = route_order_data["response"]["msgBody"][
            "busRouteStationList"
        ][arvl_bus.staOrder - 1 - arvl_bus.locationNo]["stationName"]
    ...
    for arvl_bus in bus_station.arvl_bus_list:
        ...
        print(
            "{:6} ({:>2}분) ({:>2}번째 전) {:2}     {:10}".format(
                arvl_bus.routeName,
                arvl_bus.predictTime,
                arvl_bus.locationNo,
                arvl_bus.routeTypeCd,
                arvl_bus.routeNowStaName,
            )
        )
