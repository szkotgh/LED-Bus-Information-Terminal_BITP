...
def show_everline_page(_show_time_sec: int = 15):
    ...
    font8  = config.SCD4_FONT_8
    font10 = config.SCD4_FONT_10
    font11 = config.SCD4_FONT_11
    ...
    station_title = f"[에버라인 운행정보] {info_manager.everline.STATION_CODE[base_station]}역"
    ...
    while True:
        ...
        train_infos = info_manager.everline.get_train_info()
        ...
        if train_infos != None:
            for train_info in train_infos:
                if train_info['StCode'] in [base_station, back_station, next_station]:
                    if train_info['StatusCode'] == info_manager.everline.TRAIN_START:
                        if train_info['updownCode'] == info_manager.everline.TRAIN_UPWARD:
                        ...
                        elif train_info['updownCode'] == info_manager.everline.TRAIN_DOWNWARD:
                            if train_info['StCode'] == base_station:
                    ...
                    if train_info['StatusCode'] == info_manager.everline.TRAIN_STOP:
                        if train_info['updownCode'] == info_manager.everline.TRAIN_UPWARD:
                            if train_info['StCode'] == base_station:
        ...
        draw.text((left_x, RECTANGLE_TOP+8), f"{info_manager.everline.STATION_CODE[back_station]}역", fill=(200, 200, 255, 255), font=font8)
        canvas.paste(station_icon, (left_x, center_y), station_icon)
        draw.text((center_x-5, RECTANGLE_TOP+8), f"{info_manager.everline.STATION_CODE[base_station]}역", fill=(200, 200, 255, 255), font=font8)
        canvas.paste(station_icon, (right_x, center_y), station_icon)
        draw.text((right_x-11, RECTANGLE_TOP+8), f"{info_manager.everline.STATION_CODE[next_station]}역", fill=(200, 200, 255, 255), font=font8)
        
        # Draw train interval
        train_interval = None
        if datetime.date.today().weekday()  >= 5:
            train_interval = info_manager.everline.get_train_interval(now_hhmm, True)
        else:
            train_interval = info_manager.everline.get_train_interval(now_hhmm)
        
        if train_interval != None:
            draw.text((0, matrix_manager.MATRIX_SIZE[1]-11), f"열차는 매 {train_interval}분마다 운행합니다matrix_manager", fill=(255, 255, 255, 255), font=font10)
        else:
            draw.text((0, matrix_manager.MATRIX_SIZE[1]-11), "열차가 운행하지 않는 시각입니다.", fill=(255, 255, 255, 255), font=font10)
